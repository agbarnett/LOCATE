...
# data from 0_read_data_redcap.R, should change to 1_combine_data at some time
...
approached = filter(data, !is.na(date_today), # all patients
...
for_consort = select(data, record_id, randomised, withdrew_or_excluded, reason, starts_with('date_'))
#
...
print(tab1, catDigits=0, contDigits=0, explain=TRUE, formatOptions = list(big.mark = ","))
```
...
```{r}
# to add liver size when clean

## Vector of all variables to summarise (in order of form)
myVars <- c("spleen", "fatty_liver","liver_outline")
## Vector of categorical variables
...
print(tab2, catDigits=0, contDigits=0, explain=TRUE, formatOptions = list(big.mark = ","))
```
...
```{r}
## Vector of all variables to summarise (in order of form)
myVars <- c("re_referral", "referral_cat", "referral_time")
## Vector of categorical variables
catVars <- c("re_referral", "referral_cat")
## Create a TableOne object
tab3 <- CreateTableOne(vars = myVars, 
                       ...
                       test = FALSE, 
                       addOverall = FALSE) # no total column
print(tab3, catDigits=0, contDigits=0, explain=TRUE, formatOptions = list(big.mark = ","))
```
...
to_table = select(data, ends_with('_6m')) %>% 
  ...
  tally() %>%
  group_by(name) %>%
  mutate(name = str_remove_all(name, '_6m'),
...
The table shows the counts and percentages in round brackets.
...
## Missing data at 12 month follow-up
...
_Ignore for now as 12 month follow-up is not complete._
