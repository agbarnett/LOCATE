...
plot4 = ggsurvplot(result$km, 
           ...
           ylim = c(0, 0.3), 
           legend.labs = c('New model of care','Usual care'),
           palette = group_colours,
           conf.int = FALSE,
           ggtheme = theme_bw(),
           tables.y.text = FALSE, # for alignment, use colours instead of words
          risk.table = TRUE)
plot4
jpeg('figures/time_to_primary_outcome.jpg', width=5.5, height=4.5, units='in', res=500)
print(plot4)
invisible(dev.off())
#
```
...
plot1 = ggsurvplot(result$km, 
           ...
           risk.table = TRUE)# 
plot1
jpeg('figures/time_to_fibroscan.jpg', width=5.5, height=4.5, units='in', res=500)
print(plot1)
invisible(dev.off())
...
The plot shows the estimated accumulation over time of successful scans. The vertical notches on the lines show when participants were right-censored as the follow-up time ended. The table below the graph shows the participants "at risk", meaning those who are yet to have a Fibroscan. 
The total number of participants with a scan is `r result$events`, with `r result$censored` censored.

Almost everyone in the new model of care had their scan within `r time_stat$est` days. In contrast, the usual care group slowly accumulated scans over time. 
...
plot2 = ggsurvplot(result$km, 
          ...
          risk.table = TRUE)
plot2
jpeg('figures/time_to_first_appointment.jpg', width=5.5, height=4.5, units='in', res=500)
print(plot2)
invisible(dev.off())
#
```
...
plot3 = ggsurvplot(result$km, 
          ...
          risk.table = TRUE)
plot3
jpeg('figures/time_to_ed.jpg', width=5.5, height=4.5, units='in', res=500)
print(plot3)
invisible(dev.off())
#
```
...
### Summary table of survival models

```{r}
stable = bind_rows(table_1, table_2, table_3,
                   table_4) %>%
  mutate(total_fu = round(total_fu),
   outcome = case_when(outcome == 'primary_date' ~ 'High-risk diagnosis',
       outcome == 'scan_date' ~ 'Fibroscan',
       outcome == 'date_first_appointment' ~ 'Appointment at the hepatology clinic',
       outcome == 'ed_pres_1' ~ 'ED presentation',
       TRUE ~ as.character(outcome))) %>%
  rename('Outcome' = 'outcome',
         'Number of participants' = 'n',
         'Number censored' = 'censored',
         'Total follow-up time' = 'total_fu')
flextable(stable) %>%
  theme_box() %>%
  autofit()
```

The summary table gives the number of participants, numbers censored, and total follow-up time. 

# Time between referral and randomisation

```{r}
# calculate difference
diff = mutate(data, diff = as.numeric(date_returned - date_of_referral))
hplot = ggplot(data = diff, aes(x=diff))+
  geom_histogram(fill = 'darkseagreen3', col='grey44')+
  g.theme+
  xlab('Days between referral and randomisation')+
  ylab('Number of participants')
hplot
ggsave(hplot, file='figures/time_referral_random.jpg', width=5, height=4, units='in', dpi=400)
# for text
rmedian = median(diff$diff)
```

The median time between referral and randomisation was `r round(rmedian)` days.
